...
from app.clients.database.worker_job_read_database_client import get_active_worker_job_id
from app.clients.database.worker_job_write_database_client import create_worker_job
...
from app.services.worker_version_service import resolve_rss_worker_version
...
def _enqueue_rss_scrape_job(
    ...
    *,
    ...
    status = WorkerJobStatus.COMPLETED if not feeds else WorkerJobStatus.QUEUED
    ...
    try:
        ...
        if feeds:
            enqueue_worker_tasks(
                ...
                worker_version=worker_version,
                requested_at=requested_at,
                ref_batches=[[int(feed.feed_id) for feed in batch] for batch in feed_batches],
...
def _enqueue_source_embedding_job(
    ...
    *,
    ...
    status = WorkerJobStatus.COMPLETED if not candidate_ids else WorkerJobStatus.QUEUED
